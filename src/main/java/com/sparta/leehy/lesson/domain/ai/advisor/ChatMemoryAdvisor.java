package com.sparta.leehy.lesson.domain.ai.advisor;
...
public class ChatMemoryAdvisor implements BaseAdvisor {
    ...
    public ChatClientRequest before(ChatClientRequest request, AdvisorChain chain) {
        ...
        String userText = request.prompt().getInstructions().stream()
                .filter(m -> m.getMessageType() == MessageType.USER)  // 사용자가 보낸메시지만 남김
                .map(Message::getText)
                .findFirst().orElse("");

        // 4. 합쳐진 메시지들로 프롬프트를 재구성하여 반환
        return request.mutate()   // mutate() : 뒤에 있는 메서드가 기존 요청을 덮어써줄수 있도록 도와줌
