package com.sparta.leehy.lesson.domain.ai.service;

import com.sparta.leehy.lesson.domain.ai.dto.response.ContextChatResponse;
import com.sparta.leehy.lesson.domain.ai.entity.ChatConversation;
import com.sparta.leehy.lesson.domain.ai.entity.ChatMessage;
import com.sparta.leehy.lesson.domain.ai.repository.ChatConversationRepository;
import com.sparta.leehy.lesson.domain.ai.repository.ChatMessageRepository;
import com.sparta.leehy.lesson.global.constants.enums.ChatMessageType;
import com.sparta.leehy.lesson.global.constants.enums.StatusType;
...
public class PersistentChatService {
    ...
    private static final int MAX_HISTORY_MESSAGES = 5; // 최대로 허용할 context 메세지의 개수

    @Transactional
    public ContextChatResponse chat(String conversationId, String userMessage) {
        ChatConversation conversation;
        if (!StringUtils.hasText(conversationId)) {
            // TODO : 새로운 채팅 방을 생성 해야함
    ...
    private ChatMessage saveMessage(ChatConversation conversation, String message,
        ...
        return chatMessageRepository.save(ChatMessage.builder()
                ...
                .message(message)
                .promptTokens(ObjectUtils.isEmpty(usage) ? 0 : usage.getPromptTokens())
                .completionTokens(ObjectUtils.isEmpty(usage) ? 0 : usage.getCompletionTokens())
                .totalTokens(ObjectUtils.isEmpty(usage) ? 0 : usage.getTotalTokens())
                .build());
    }
