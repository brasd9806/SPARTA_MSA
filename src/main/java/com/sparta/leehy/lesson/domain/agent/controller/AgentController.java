package com.sparta.leehy.lesson.domain.agent.controller;

import com.sparta.leehy.lesson.domain.agent.dto.request.AgentRequest;
import com.sparta.leehy.lesson.domain.agent.dto.request.LimitedAgentRequest;
import com.sparta.leehy.lesson.domain.agent.dto.request.ReflectionRequest;
import com.sparta.leehy.lesson.domain.agent.dto.response.AgentResponse;
import com.sparta.leehy.lesson.domain.agent.service.AgentService;
...
public class AgentController {
    ...
    /**
     * 2. 제한된 도구만 사용하는 Agent
     * 특정 목적을 위해 허용된 도구 리스트를 시스템 프롬프트로 명시합니다.
     */
    @PostMapping("/solve/limited")
    public ApiResponse<AgentResponse> solveWithLimitedTools(@RequestBody LimitedAgentRequest request) {
        log.info("[API] Limited Tools: {}, Tools: {}", request.getGoal(), request.getTools());

        String result = agentService.solveWithLimitedTools(request.getGoal(), request.getTools());

        return ApiResponse.ok(AgentResponse.builder()
                .goal(request.getGoal())
                .usedTools(request.getTools())
                .result(result)
                .build());
    }

    /**
     * 3. 계획-실행(Plan-and-Execute) Agent
